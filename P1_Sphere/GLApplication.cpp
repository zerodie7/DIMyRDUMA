...
void GLApplication::initialize() {
	if (!windowManager
			|| !windowManager->initialize(800, 700, "FPS", false)) {
		this->destroy();
		exit(-1);
	}
	...
	shader.initialize("Shaders/sistemascoordenados.vs", "Shaders/sistemascoordenados.fs");
...
void GLApplication::applicationLoop() {
	...
	while (processInput) {
		...
		glm::mat4 view = glm::lookAt(cameraPos, cameraLookAt,
			glm::vec3(0, 1, 0));
			*/
		...
		// Transformation FPS
		glm::mat4 view;
		view = glm::lookAt(windowManager->inputManager.getCameraPos(), windowManager->inputManager.getCameraPos() 
			+ windowManager->inputManager.getCameraFront(),
...
//PROJECTION
		...
		for (GLuint i = 0; i < 10; i++) {
			...
			model = glm::rotate(model, angle, glm::vec3(1.0f, 0.3f, 1.5f));
